...
const levels = require("../../Databases/Schema/levels.js");
var mongoUrl;

class DiscordXp {
  static async setURL(dbUrl) {
    if (!dbUrl) throw new TypeError("Một url cơ sở dữ liệu không được cung cấp.");
    mongoUrl = dbUrl;
  ...
  static async createUser(userId, guildId) {
    if (!userId) throw new TypeError("");
    if (!guildId) throw new TypeError("");
    ...
    await newUser.save().catch(e => console.log(``));
  ...
  static async deleteUser(userId, guildId) {
    ...
    await levels.findOneAndDelete({ userID: userId, guildID: guildId }).catch(e => console.log(``));
  ...
  static async appendXp(userId, guildId, xp) {
    if (!userId) throw new TypeError("");
    if (!guildId) throw new TypeError("");
    if (xp == 0 || !xp || isNaN(parseInt(xp))) throw new TypeError("");
    ...
    await user.save().catch(e => console.log(``) );
  ...
  static async appendLevel(userId, guildId, levelss) {
    if (!userId) throw new TypeError("");
    if (!guildId) throw new TypeError("");
    if (!levelss) throw new TypeError("");
  ...
  static async computeLeaderboard(client, leaderboard, fetchUsers = false) {
    if (!client) throw new TypeError("Một khách hàng không được cung cấp.");
  ...
  static xpFor (targetLevel) {
    if (isNaN(targetLevel) || isNaN(parseInt(targetLevel, 10))) throw new TypeError("Mức mục tiêu phải là một số hợp lệ.");
    if (isNaN(targetLevel)) targetLevel = parseInt(targetLevel, 10);
    if (targetLevel < 0) throw new RangeError("Mức mục tiêu phải là một số dương.");
    return targetLevel * targetLevel * 100;
  }

   static async deleteGuild(guildId) {
    if (!guildId) throw new TypeError("Id guild không được cung cấp.");

    const guild = await levels.findOne({ guildID: guildId });
    if (!guild) return false;

    await levels.deleteMany({ guildID: guildId }).catch(e => console.log(`Không xóa được bang hội: ${e}`));
