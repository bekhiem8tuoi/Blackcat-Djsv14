...
class JoshProvider {
  constructor(options) {
    if (!options.name) {
      throw new Err('Phải cung cấp các tùy chọn. Tên', 'JoshTypeError');
    }
    this.name = options.name;
    if (!options.collection) {
      throw new Err('Phải cung cấp các tùy chọn. Bộ sưu tập', 'JoshTypeError');
    }
  ...
  async set(key, path, val) {
    await this.check();
    if (!key) {
      throw new Error('Các phím phải là chuỗi hoặc số.');
    }
  ...
  async check(key, type, path = null) {
    ...
    if (isNil(value)) {
      throw new Err(
        `The document "${key}" of path "${path}" was not found in the database`,'JoshTypeError',);
    }
