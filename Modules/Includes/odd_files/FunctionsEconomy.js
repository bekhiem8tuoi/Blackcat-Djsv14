...
const cs = require("../../Databases/Schema/currency");
const inv = require('../../Databases/Schema/inventory');
...
async function saveUser(data, data2) {
    ...
    process.nextTick(async () => {
        ...
        if (data2) data2.save(_ => _ ? console.error(`LỖI Xảy ra khi lưu dữ liệu (Hệ thống tiền tệ) \n${'='.repeat(50)}\n${_+'\n'+'='.repeat(50)}`) : 'Không có lỗi');
    }, data, data2);
...
function updateInventory(mongoURL, newData, settings, collection = "inventory-currencies") {
    event.emit('debug', `[ CS => Debug ] : Chức năng Cập nhật khoảng không quảng cáo được thực thi.`)
    if (typeof settings.user === 'string') settings.user = {
        id: settings.user
    }
    if (typeof settings.guild === 'string') settings.guild = {
    ...
    new(require('mongodb').MongoClient)(mongoURL, {
        ...
        if (err) return event.emit('debug', `[ CS => Error ] : Không thể kết nối với MongoDB ( cập nhật Chức năng Kiểm kê )`, err)
        ...
        db.db(mongoURL.split('/')[mongoURL.split('/').length - 1]).collection(collection).updateOne(query, {
            ...
            if (err) return event.emit('debug', `[ CS => Error ] : Không thể lưu dữ liệu vào MongoDB ( cập nhật Chức năng Kiểm kê )`, err)
            if (res.result.n) event.emit('debug', `[ CS => Debug ] : Dữ liệu được Lưu thành công ( cập nhật Chức năng Kiểm kê )`);
            else event.emit('debug', `[ CS => lỗi ] : MongoDB đã không cập nhật DB. ( cập nhật Chức năng Kiểm kê )`);
            db.close();
            event.emit('debug', `[ CS => Debug ] : Đóng DB  ( cập nhật Chức năng Kiểm kê )`)
        });
    });
};
