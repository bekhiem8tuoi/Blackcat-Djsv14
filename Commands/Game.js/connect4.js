...
class ConnectFour {
    constructor(options) {
        if (!options.message) throw new TypeError('Thiếu đối số: message')
        this.gameEmbed = null
    ...
    start() {
        ...
        if (!oppenent) return this.message.channel.send(`${client.i18n.get(language, "game", "connect_3")}`)
        ...
        const initial = new EmbedBuilder()
            .setTitle(`${this.player1} ${client.i18n.get(language, "game", "connect_2")}, ${this.message.author.username}!`)
                ...
                if (user.id === gameData[player].member.id) {
                    ...
                    switch (reaction.emoji.name) {
                        case "1️⃣":
                            ...
                            if (openSpaces.length == 0) return this.message.channel.send(`**${gameData[player].member}, ${client.i18n.get(language, "game", "connect_4")}**`).then(msg1 => msg1.delete({ timeout: 3000000 }))
                    ...
                    if (tieCheck()) {
                        ...
                        const TieEmbed = new EmbedBuilder()
                            .setTitle(`${client.i18n.get(language, "game", "connect_0")}`)
                    ...
                    for (const func of checks) {
                        ...
                        if (data) {
                            ...
                            const WinEmbed = new EmbedBuilder()
                                .setTitle(`${client.i18n.get(language, "game", "connect_1", { connect_user_win: gameData[player].member.username})}`)
                                .setDescription(renderBoard(board))
                                .setFooter({ text: `${challenger.username} vs ${oppenent.username}`})
                    ...
                    const newEmbed = new EmbedBuilder()
                        .setTitle(`${gameData[player].playerColor} - ${client.i18n.get(language, "game", "connect_2")}, ${gameData[player].member.username}!`)
                        .setDescription(renderBoard(board))
                        .setFooter({ text: `${challenger.username} vs ${oppenent.username}`})
