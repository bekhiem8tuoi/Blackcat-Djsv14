...
module.exports = {
    ...
    aliases: ["", ""], // lệnh phụ
    description: "Phát một bài hát/danh sách phát và thêm nó vào đầu!", // mô tả lệnh
    ...
    try {
      ...
      if (!args[0]) {
				return message.reply({ embeds: [new EmbedBuilder()
						...
						.setDescription(`${client.i18n.get(language, "music", "playtop_0")}`)
					],
				});
			}
			const Text = args.join(" ");
			let newmsg = await message.reply({ content: `${client.i18n.get(language, "music", "play_2", { play_emoji: emoji.Meow, play_Text: Text })}`}).catch(e => {console.log(e)})
			try {
				...
				newmsg.edit({ content: `${queue?.songs?.length > 0 ? `${client.i18n.get(language, "music", "playtop_1")}` : `🎶 ${client.i18n.get(language, "music", "play_4")}:`}: \`\`\`css\n${Text}\n\`\`\``}).catch(e => {console.log(e)});
			} catch (e) {
